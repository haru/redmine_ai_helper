...
When `all_projects_scope` is off, both methods are behaviorally equivalent to the pre-existing logic they replace (`data_accessible?` reduces to `visible? && module_enabled? && allowed_to?(:view_ai_helper)`; `data_access_condition` ANDs `Project.allowed_to_condition(user, :view_ai_helper)` with `Project.visible_condition(user)`, which is redundant for every caller because those callers already apply data-type visibility — `AND` is idempotent) — this is the FR-003 compatibility guarantee, not just a testing convention.

`data_access_condition` is deliberately self-contained: it always ANDs `Project.visible_condition(user)` with the module/scope part, so a future caller that forgets to apply `Issue.visible(user)` (or a refactor that moves it) cannot silently widen the result set — the failure mode would otherwise be an over-permissive query that raises no exception and logs nothing. The extra visibility AND is harmless for correct callers.
...
`AiHelperSetting`'s vector-registration scope (`#vector_scope_projects`, `#vector_target?`) is widened the same way and by the same flag, for the same reason ADR-002's registration scope has always mirrored the data-access scope: a project excluded from data access has no reason to have its content sitting in the vector database. When `all_projects_scope` is on, `#vector_scope_projects` returns all **non-archived** projects — the vector database holds content for the sole purpose of answering data-access requests, so a project outside the data-access scope (archived projects are out of scope per the feature spec) must not accumulate embeddings; the existing cleanup path removes them from scope the moment the setting is turned off. Closed (read-only) projects stay in the registration scope because they remain readable data.

## Scope Notes: Archived and Closed Projects

Two project states behave asymmetrically and this is intentional (see the feature spec's boundary rules):

- **Archived** (`status = 9`): never in the data-access or vector-registration scope when `all_projects_scope` is on. `data_access_condition` excludes them via `Project.visible_condition` (`:view_project` is a read permission, so its base statement only admits active and closed projects), `data_accessible?` excludes them via `project.visible?(user)`, and `vector_scope_projects`/`vector_target?` exclude them explicitly.
- **Closed** (`status = 5`): treated as readable data, per Redmine convention. The module-**enabled** branch of `data_access_condition` (`Project.allowed_to_condition(user, :view_ai_helper)`) still excludes closed projects — `:view_ai_helper` is deliberately not declared `read: true`, so its base statement is `status = 1` only — while the module-**disabled** branch (`NOT EXISTS`) admits closed projects through `visible_condition`'s `status IN (active, closed)`. So under `all_projects_scope`: a closed module-enabled project is denied, a closed module-disabled project is allowed (subject to visibility). Read-only data, no write path is widened by this ADR; a future maintainer must preserve this asymmetry along with the module-enabled/module-disabled one above.
