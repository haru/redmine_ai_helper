...
(S024). Recorded as decision 2 of **ADR-022** (Accepted 2026-08-14) (S025).
...
for the same `channel_type`/`channel_id` (S024):
...
- It needs no new table and no change to shared components — the whole point of
  SC-007's "one adapter class" claim (S024). The access is **read-only**: the
  judgement writes nothing and changes no schema, which is what keeps it clear
  of the shared queue's lifecycle (S025).
- The evidence lives in the database, so the boundary survives a gateway restart,
  which an in-memory last-message timestamp would not (S024).
- `ai_helper_inbound_events` retains rows for 7 days. If retention removes the
  ...
  (S024). ADR-022 books this as a **negative** consequence all the same: the
  ...
  intended failure direction, but a behaviour operators may notice (S025).
...
counts a scan is harmless, and the alternative is modifying a shared table (S024).
