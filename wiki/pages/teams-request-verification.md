...
drop every request until the cache expires (S024).
...
The adapter's other two caches are instance-level (S025); see
...
stops a stolen token from redirecting answers to another service URL (S024).
...
as was trusting `tenant.id` alone, which anyone can forge (S024).
...
does (S026). With the bot registered as single-tenant (**ADR-023**) no outside
...
(S025). Placing the check in `verify_request` also settles
the rejection before the event is queued, which is what guarantees no answer is
ever generated for a foreign tenant (S024).
...
the gateway at reply time, which leaves a larger attack surface (S024).
...
are never logged (S024).

Both gates are decision 1 of **ADR-022** (Accepted 2026-08-14): an unauthorized
organization never reaches the queue, let alone an answer — the check holds
before any LLM work, any queue row, and any log of question content (S025).
